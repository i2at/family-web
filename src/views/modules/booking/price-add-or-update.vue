<template>
  <el-dialog
    ...
    <el-form-item label="价格变化时间" prop="changeTime">
      <el-input v-model="dataForm.changeTime" placeholder="价格变化时间"></el-input>
    </el-form-item>
    ...
    data () {
      return {
        ...
        dataRule: {
          ...
          changeTime: [
            { required: true, message: '价格变化时间不能为空', trigger: 'blur' }
          ],
          description: [
    ...
    methods: {
      init (id) {
        ...
        this.$nextTick(() => {
          ...
          if (this.dataForm.id) {
            this.$http({
              url: this.$http.adornUrl(`/booking/price/info/${this.dataForm.id}`),
              method: 'get',
              params: this.$http.adornParams()
            }).then(({data}) => {
              if (data && data.code === 0) {
                this.dataForm.categoryId = data.price.categoryId
                this.dataForm.categoryName = data.price.categoryName
                this.dataForm.unitId = data.price.unitId
                this.dataForm.unitName = data.price.unitName
                this.dataForm.price = data.price.price
                this.dataForm.changeTime = data.price.changeTime
                this.dataForm.description = data.price.description
                this.dataForm.createrId = data.price.createrId
                this.dataForm.creater = data.price.creater
                this.dataForm.createTime = data.price.createTime
                this.dataForm.updaterId = data.price.updaterId
                this.dataForm.updater = data.price.updater
                this.dataForm.updateTime = data.price.updateTime
                this.dataForm.showStatus = data.price.showStatus
                this.dataForm.sort = data.price.sort
                this.dataForm.firstLetter = data.price.firstLetter
              }
            })
          }
        })
      ...
      dataFormSubmit () {
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            this.$http({
              url: this.$http.adornUrl(`/booking/price/${!this.dataForm.id ? 'save' : 'update'}`),
              method: 'post',
